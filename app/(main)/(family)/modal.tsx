...
export default function Modal() {
    ...
    const handleAddFamilyMember = async () => {
        ...
        let memberRelation = selectedRelation === "Outro" ? relationRef.current.trim() : selectedRelation

        const { data, error } = await supabase
        .from("FamilyMembers")
        .insert({name: memberName, age: memberAge, phone: memberPhone, relation_type: memberRelation})
    ...
    const handleUpdateFamilyMember = async () => {
        let relation_type = selectedRelation === "Outro" ? relation : selectedRelation
  ...
  return (
    <ScreenWrapper>
        <View style={styles.container}>
            ...
            <Input
            placeholder="Nome"
            onChangeText={(value: string) => memberInfo.name ? setName(value) : nameRef.current = value}
            ...
            placeholder="Idade"
            inputMode="numeric"
            onChangeText={(value: string) => memberInfo.age ? setAge(value) : ageRef.current = value}
            value={age}
            />
            ...
            onChangeText={(value: string) => memberInfo.phone ? setPhone(value) : phoneRef.current = value}
            value={phone}
            />
            ...
            <View style={selectedRelation === 'Outro' ? {display: 'flex'} : {display: 'none'}}>
                <Input placeholder="Informe o parentesco..." onChangeText={(value: string) => memberInfo.relation_type ? setRelation(value) : relationRef.current = value}  />
            </View>
            

            <Button title="Adicionar" buttonStyle={buttonTitle ? {display: 'none'} : {display: 'flex'}} onPress={handleAddFamilyMember} />
            <Button title="Editar" buttonStyle={buttonTitle ? {display: 'flex'} : {display: 'none'}} onPress={handleUpdateFamilyMember} />
        </View>
    </ScreenWrapper>
